    ...
    <GlassCard className="overflow-hidden">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 text-sm text-slate-600">
              <GlassChip className="flex items-center gap-2"><Mail className="h-4 w-4" />Coach</GlassChip>
      ...
      <CardContent className="space-y-5 p-6">
        <div className="grid gap-3 md:grid-cols-3">
          <GlassMetric label="Weekly sessions" value={`${request.weeklyTrainingCount}`} description="Requested weekly workout count." />
          <GlassMetric label="Location" value={request.preferredLocation} description="Preferred place for the training plan." />
          <GlassMetric label="Status" value={statusLabelMap[request.status] ?? request.status} description="Current state of your training request." />
        </div>
        ...
        {(request.status === "APPROVED" || request.status === "CLOSED") && trainingPlanDescription ? <RequestTextPanel label="Training plan description" value={trainingPlanDescription} /> : null}
        {(request.status === "APPROVED" || request.status === "CLOSED") && trainingPlanFileName ? (
          <GlassCardSoft><CardContent className="p-5 text-sm"><p className="text-xs uppercase tracking-[0.32em] text-slate-500">Attached file</p><p className="mt-3 text-slate-600">{trainingPlanFileName}</p></CardContent></GlassCardSoft>
        ) : null}
